...
class ChartingState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		if (!typingShit.hasFocus)
		{
			...
			if (FlxG.keys.justPressed.N){
				this.noteType -= 1;
				if (noteType < 0)
					noteType = types.length - 1;
				trace("Changed to " +  typeText);
			}
			if (FlxG.keys.justPressed.M){
				this.noteType += 1;
				if (noteType == types.length)
					noteType = 0;
				trace("Changed to " +  typeText);
			}
	...
	function updateGrid():Void
	{
		...
		for (section in _song.notes)
		{
			for (i in section.sectionNotes)
			{
				...
				var note:Note = new Note(daStrumTime, daNoteInfo % 4,null,false,true,i[3], daType);
	...
	private function addNote(?n:Note):Void
	{
		...
		var noteSus = 0;
		var noteType = types[this.noteType];

		if (n != null)
			section.sectionNotes.push([n.strumTime, n.noteData, n.sustainLength, false]);
		else
			section.sectionNotes.push([noteStrum, noteData, noteSus, noteType, false]);
